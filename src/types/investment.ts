...
export interface Investment {
  ...
  // ── "ซื้อเพิ่มแล้วรอบนี้" — ปิดแจ้งเตือนจนกว่าจะแดงครบรอบใหม่ ──
  // ลงไม้ตามสัญญาณแล้วการ์ดยังเตือนค้างอยู่ = อ่านไม่ออกว่าอันไหนทำแล้ว/ยังไม่ทำ
  // เก็บ "จำนวนแท่งตอนที่กดปิด" คู่กับ "เวลาเปิดของแท่งแรกในสตรีคนั้น" —
...
export interface UserCurrency {
  ...
  /**
   * ค่าธรรมเนียม "มาตรฐาน" ของตลาดที่ซื้อขายด้วยสกุลนี้ (% ต่อคำสั่ง)
   * ตั้งที่นี่ครั้งเดียวแล้วทุกแพลตฟอร์มที่ยังไม่ได้ตั้งของตัวเองจะใช้ตัวนี้
   * — กันไม่ให้ต้องพิมพ์เลขเดิมซ้ำทุกโบรกที่เทรดตลาดเดียวกัน
   */
  feePercent?: number;
  /** ขั้นต่ำต่อคำสั่ง **ในสกุลนี้เอง** (USD ก็คือดอลลาร์ ไม่ใช่บาท) */
  feeMin?: number;
  createdAt: string;
}
...
export interface UserPlatform {
   ...
   */
  feePercent?: number;
  /** ค่าธรรมเนียมขั้นต่ำต่อคำสั่ง — หน่วยตาม `feeMinCurrency` (ค่าเริ่มต้นคือบาท) */
  feeMinTHB?: number;
  /**
   * สกุลของ `feeMinTHB` — ไม่ตั้ง = บาท (ชื่อฟิลด์ยังลงท้าย THB เพราะคอลัมน์เดิมชื่อนั้น
   * และ 99% ของข้อมูลเก่าคือบาทจริง ๆ เปลี่ยนชื่อคอลัมน์ = ต้องย้ายข้อมูลโดยไม่ได้อะไรเพิ่ม)
   *
   * มีไว้เพราะโบรกต่างประเทศคิดขั้นต่ำเป็นดอลลาร์ (IBKR $1/คำสั่ง) — บังคับกรอกเป็นบาท
   * แปลว่าต้องแปลงเองทุกครั้งที่เรตขยับ ซึ่งไม่มีใครทำจริง
   */
  feeMinCurrency?: string;
  createdAt: string;
}
