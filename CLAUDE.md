...
| `src/services/*.ts` (27) | Supabase I/O + external APIs → §4 |
...
| `sql/*.sql` (22) | Hand-run schema, idempotent → §8 |
...
| `ladderFreedStorage` | `expense_ladder_freed` | §6.6. เก็บ "ขั้นบันไดที่คนกดยืนยันว่าปลดจริง" — **ตารางแยกโดยตั้งใจ** เพราะบันไดเป็นมุมมองคร่อม `life_costs` + `recurring_bills` สถานะของบันไดจึงเป็นของบันได ไม่ใช่ของรายการต้นทาง (และ `recurring_bills` เป็นตารางฐานที่ไม่มี SQL ในรีโป). คีย์คือ `(user_id, item_kind, item_id)` — ต้องมี `kind` เพราะ id ของสองตารางชนกันได้. `getLadderFreed` คืน `Map<'kind:id', วันที่>` และ **ทนตารางหายด้วยการคืน Map ว่าง** ไม่ throw — "ยังไม่มีใครกด" กับ "ยังไม่รัน SQL" ให้ผลบนจอเหมือนกันพอดี. **ห้ามให้ค่าจากตารางนี้ไปคุมคณิตบันได** (ดู §6.6) |
...
| `expenseLadder.ts` (175) | **ด่านที่ต้องผ่านก่อนเป้าอื่นทั้งหมด** — ค่าเสื่อม + บิลประจำคือของที่ต้องจ่ายทุกเดือนเหมือนกัน จึงอยู่บันไดเดียวกัน เรียงจากทุนน้อยไปมากเพื่อให้ปลดอันแรกเร็วที่สุด แล้วเงินที่เคยจ่ายอันนั้นว่างมาเร่งขั้นถัดไป (debt snowball กลับด้าน). ⚠️ **ทุนที่ต้องมี = ยอดต่อปี ÷ (ผลตอบแทน − เงินเฟ้อ)** ไม่ใช่หารด้วยผลตอบแทนเปล่า ๆ — ค่าเน็ตวันนี้ไม่ใช่ค่าเน็ตอีก 10 ปี ที่ 7%/2.5% ทุนต่างกันถึง 56% พลาดตรงนี้แล้วตัวเลขให้ความมั่นใจเกินจริง. ผลตอบแทน ≤ เงินเฟ้อ = คืน `reason` ไม่ใช่ Infinity. `avgMonthlyBill` ใช้ **ยอดที่กรอกจริงเฉลี่ย** จาก `monthlyAmounts` ไม่ใช่ `RecurringBill.amount` ที่เป็นแค่ค่าอ้างอิง (§4.1). ⚠️ **"ถึงแล้ว" (`reached`) กับ "ปลดแล้ว" (`freedAt`) เป็นสองสถานะ ห้ามยุบเป็นอันเดียว** (เจ้าของสั่ง 2026-08-22): `reached` = ทุนพอตามคณิต · `freedAt` = คนกดยืนยันว่าเปลี่ยนมาให้พอร์ตจ่ายจริงแล้ว (ตาราง `expense_ladder_freed`). เดิมมีสถานะเดียวแล้วพิมพ์ "ปลดแล้ว" ทันทีที่ทุนถึง ซึ่ง**อ้างสิ่งที่ยังไม่เกิด** — เจ้าของยังจ่ายจากเงินเดือนอยู่ ยังไม่เคยขายอะไรเลย (หลักเดียวกับ `achievedAt` ของด่านชีวิต และ `powderLegsUsed`: แอปคำนวณ คนกดยืนยัน). ⚠️ **`freedAt` ห้ามย้อนไปคุมคณิต** — `current`/ลำดับสะสมคิดจาก `reached` อย่างเดียว ถ้าเอาการกดยืนยันไปคุม ขั้นที่ถึงแล้วแต่ยังไม่กดจะทำให้โซ่สะสมขาดกลางแล้วขั้นถัดไปเด้งเป็น "ขั้นที่กำลังทำ" ทั้งที่ทุนยังไม่ถึง. `freedMonthlyTHB` นับจาก **freed เท่านั้น** (ประโยคนี้อ้างว่าเงินอยู่ในกระเป๋าจริง) ส่วน `reachedNotFreedMonthlyTHB` มีไว้ให้จอชวนกด. **ปุ่มยืนยันอยู่หน้าค่าเสื่อมที่เดียว** หัวพอร์ตอ่านอย่างเดียว. ⚠️ ปุ่มต้องโผล่เมื่อ `reached || freedAt` — เช็คแค่ `reached` แล้วสลับชิปผลตอบแทนลง ขั้นที่กดไว้จะกลายเป็นยังไม่ถึงและปุ่มยกเลิกหายไปทั้งที่ยังติ๊กอยู่ (เจอตอนเรนเดอร์จริง). **ทุกตัวเลขมีคู่แฝดหน่วย "ต่อเดือน"**: `cumulativeMonthlyTHB` / `affordableMonthlyTHB` / `monthlyProgressPercent` — คิดกลับจากทุน (`× r ÷ 12`) ไม่ใช่บวก `monthlyTHB` ซ้ำ เพื่อไม่ให้สองหน่วยหลุดจากกันได้ · `affordableMonthlyTHB` ใช้ผลตอบแทน**หลังเงินเฟ้อ** จึงเป็น "จ่ายได้ตลอดไปโดยไม่แตะเงินต้น" ไม่ใช่ผลตอบแทนที่ได้เดือนนี้ — ห้ามเขียนป้ายผิด. **`outflowsFrom` (2026-08-22) เป็นตัวประกอบลิสต์ที่เดียวของทั้งแอป** — หัวพอร์ตใช้บันไดชุดนี้เป็นเป้าพอร์ต (§7.2) ถ้าสองจอประกอบลิสต์เอง จะบอก "ปลดครบต้องมีเท่าไหร่" ไม่ตรงกัน. |
...
|---|---|---|---|
| `PortfolioScreen` | **3199** | PortfolioTab | Still the heaviest file and the hub for 10 routes, but now only three things render here: portfolio header + goal, the **"ถึงคิวลงไม้"** red-candle card (kept because it is the one thing to act on today), and the holdings list + filters. Everything else is a `MenuRow` (`components/MenuRow`, module scope — §1.13) into its own screen. **Menu order (2026-08-22) puts the goal first, then the order you act in:** บัญชีให้พอร์ตจ่ายชีวิต (§6.8 — เป้าที่มาก่อนเป้าอื่น, โชว์ตลอดแม้ยังไม่เริ่ม) → เงินรอลงทุน → รอบลงทุน → บันทึกสัญญาณ → ปลดล็อกรางวัล → ภาษีจากกำไรที่ขาย (the last two only render when there is something in them). The `first` flag moved off เงินรอลงทุน onto the ledger row — two rows claiming `first` doubles the top border, none claiming it leaves a stray divider against the card edge. **`ผลงานที่ขายแล้ว` is no longer a row here** — it moved into `CyclesScreen`; the รอบลงทุน row carries the realized P/L on its sub-line so the entrance does not vanish from the hub. The refresh button lives **inside the header status line** next to the countdown (`PriceRefreshStatus`), not in the icon row — that row is only add / group-by-platform / manage-catalog now. **เป้าหมายพอร์ตมีสองที่มา (2026-08-22 เจ้าของเลือกเอง) — ชิป `ปลดค่าชีวิต` / `เป้าที่ตั้งเอง`, ค่าเริ่มต้นคือ `ปลดค่าชีวิต`** เพราะเลขนั้นตอบได้ว่า "ทำไมต้องเป็นตัวเลขนี้" ส่วนยอดที่พิมพ์เองตอบไม่ได้. โหมด `ปลดค่าชีวิต` เอา `buildExpenseLadder(outflowsFrom(...))` (§6.6) มาเป็นเป้า. ⚠️ **หน่วยของเป้าคือ บาท/เดือน ไม่ใช่ก้อนทุน** (เจ้าของสั่งแก้ 2026-08-22 หลังเห็นเวอร์ชันทุน): `เป้า เดือนละ X` = `ladder.totalMonthlyTHB` · ฝั่งซ้าย `ถ้าขายตอนนี้ จ่ายได้เดือนละ Y` = `ladder.affordableMonthlyTHB` (มูลค่าพอร์ต × ผลตอบแทนจริง ÷ 12 = จ่ายได้ตลอดไปโดยไม่แตะเงินต้น) · แถบ = `monthlyProgressPercent`. "เดือนละ 12,000" อ่านออกทันที ส่วน "ทุน 1,354,356" ต้องแปลในหัวก่อน — ทุนเลยไปอยู่บรรทัดข้อสมมติแทน ไม่ใช่พระเอก. แถวในลิสต์มี **สามสถานะ**: ยังไม่ถึง = `%` · ทุนถึงแล้วยังไม่กดยืนยัน = `ถึงแล้ว` (ทอง) · กดแล้ว = `ปลดแล้ว` (เขียว) — ปุ่มกดยืนยันอยู่หน้า `LifeCost` เท่านั้น (§6.6). ⚠️ **บล็อกนี้โฟกัส "ขั้นเดียว" ไม่ใช่ทั้งบันได (เจ้าของสั่งแก้ 2026-08-22 รอบสอง)**: หัวเป็น `ขั้นที่ N จาก M · ชื่อ` + `เดือนละ <ยอดของขั้นนี้>` และ **แถบวัด `rung.percent` (ความคืบหน้าของขั้นนี้) ไม่ใช่ `monthlyProgressPercent` ของทั้งบันได** — ทั้งบันไดขยับทีละเศษจนดูเหมือนไม่ไปไหน (16% ที่ขยับปีละไม่กี่ %) ส่วนขั้นนี้ 25% แล้วและขยับเห็นได้จริง; ตรงกับหลักบันไดด้วย (ปลดอันถูกสุดก่อน ไม่ใช่ไล่ทั้งกองพร้อมกัน). **ยอดก้อนที่โชว์ต้องเป็นก้อนของขั้นนี้** (`rung.cumulativeTHB − totalValue` = "เพิ่มอีกเท่าไหร่") ซึ่งเป็นเลขที่ไปถึงได้จริง ไม่ใช่ทุนปลดครบหลักล้าน. ยอดรวมทั้งบันไดไปอยู่บรรทัดข้อสมมติ ไม่ได้หายไป. **ลิสต์รายอย่างเรียงจากน้อยไปมากยังอยู่ แต่พับไว้** (`ladderListOpen`, ปุ่ม `ดูทั้ง N อย่าง`) เพื่อไม่ให้แย่งความสนใจจากขั้นที่กำลังทำ — กางแล้วขั้นปัจจุบันตัวหนา (`ladderNameNow`); เรียงถูกอยู่แล้วจาก `buildExpenseLadder` (ทุนน้อย = ยอดต่อเดือนน้อย) ตัดที่ `LADDER_ROWS_MAX` **8** แถวแล้ว**พิมพ์บอกว่าเหลืออีกกี่อย่าง ห้ามตัดเงียบ**. ⚠️ **จำนวน "ปลดไปแล้ว" ไหวตัวตามอัตราผลตอบแทนอย่างรุนแรง** — ที่ 25% ได้ 2/4 แต่ที่ 7% ได้ 0/4 (ทุนเดียวกัน) จึงต้องพิมพ์อัตราที่ใช้กำกับไว้เสมอ ห้ามโชว์จำนวนลอย ๆ; วัดด้วย **มูลค่าพอร์ต** (`totalValue`) ให้ตรงกับ `LifeCostScreen`; อัตราผลตอบแทนใช้ `goal.expectedAnnualReturnPercent` ถ้าตั้งไว้ ไม่งั้น `LADDER_DEFAULT_RETURN` **7** (ต้องเท่ากับค่าเริ่มต้นของชิปในหน้าค่าเสื่อม) และต้องพิมพ์กำกับว่าเป็นข้อสมมติ; `ladder.reason != null` (ผลตอบแทนไม่ชนะเงินเฟ้อ) = ใช้เป็นเป้าไม่ได้ ต้องถอยไปโหมดตั้งเอง. ปุ่มมุมขวาในโหมดนี้พาไป `LifeCost` ไม่ใช่โมดัลแก้เป้า — เป้ามาจากค่าใช้จ่ายจริง ไม่ใช่ตัวเลขที่ตั้งได้. ชิปโผล่ทันทีที่บันไดคิดได้ ไม่รอให้มีเป้าที่ตั้งเองก่อน (ไก่กับไข่). โหมดตั้งเองยังทำงานเหมือนเดิมทุกอย่าง โชว์ target **amount** ไม่ใช่ `% ของเป้า` และไม่มีรายละเอียดพับ. เลือกโหมดเก็บใน state ของจอ **ไม่ลง DB** — เป็นการสลับดู ไม่ใช่การตั้งค่าผูกมัด. Price refresh (`PRICE_REFRESH_MS` 5 min staleness), grid math (`GRID_COL_TARGET` 380, `GRID_MAX_COLS` 6, `CARD_GRID_BASIS` 520), sell modal + portfolio-goal modal. It still loads `realizedTrades`/`cycles`/`plan`/`purchaseGoals`/`taxProfile` — **only to fill the summary numbers on the menu rows**; the editing lives in the child screens. Mount skips `refreshIfStale` — `useFocusEffect` already ran `loadData()`. |
...
Tables → owner: `expenses` + `recurring_bills` (`storage`), `incomes` (`incomeStorage`), `investments` + `transactions` (`investmentStorage`), `realized_trades` (`realizedStorage`), `tax_profiles` (`taxStorage`), `user_profile` (`userProfileStorage`), `activity_log` (`activityLogStorage`), `investment_cycles` (`cycleStorage`), `red_signals` (`redSignalStorage`), `life_costs` (`lifeCostStorage`), `life_goals` (`lifeGoalStorage`), `life_ledger` (`lifeLedgerStorage`), `expense_ladder_freed` (`ladderFreedStorage`), `investment_plan` + `portfolio_goals` (singletons), `purchase_goals`, `installment_plans`, `accounts`, `account_transfers` (written only by `importStorage`), `user_currencies`, `user_platforms`, `telegram_pending` (edge fn only).
